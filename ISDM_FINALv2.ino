...
#define pressureSeaLevel 102000   //pressure at sea level
...
#define voltage0ms  2.26          //voltage at 0 m/s
#define B 0.23                     //B constant
...
void processData(){
  ...
  if(transistor == true){  
    if(i == 4){
      ...
      airvelocity = ((voltage*voltage)-(voltage0ms*voltage0ms))/B;
      airvelocity = pow(airvelocity,2);
      temp = voltage;    
    } else i ++;
  } else {
    voltage = (float)valueS3*5/1023;
  }
  int c = (1E12/(1.4*frq*430000))-158;
  humidity = 2*c-695;
